...
if (window.localStorage.getItem('custom_setdex') != null) {
    SETDEX_CUSTOM = JSON.parse(window.localStorage.getItem('custom_setdex'))
    reloadXYScript()
}
var deletecustom = function () {
    SETDEX_CUSTOM = {}
    window.localStorage.removeItem('custom_setdex')
    reloadXYScript()
}
...
var savecustom = function () {
    ...
    if (spreadName == '') spreadName = 'My Custom Set'
...
9	- Move Name
	*/
    ...
    } else species = lines[0].split('@')[0].trim() //species is always first
    for (var i = 0; i < showdownFormes.length; ++i) {
        if (species == showdownFormes[i][0]) species = showdownFormes[i][1]
    }
    ...
    if (lines.length > 2) {
        for (var i = 2; i < lines.length; ++i) {
            ...
            if (lines[i].indexOf('EVs') != -1) {
                //if EVs are in this line
            ...
            if (lines[i].indexOf('Nature') != -1) {
                //if nature is in this line
                nature = lines[i].split(' ')[0].trim()
            }
            if (lines[i].indexOf('- ') != -1) {
                //if there is a move in this line
                var nextMove = lines[i]
  ...
  */
    ...
    window.localStorage.setItem('custom_setdex', JSON.stringify(SETDEX_CUSTOM))
    reloadXYScript()
}
