function getKOChanceText(damage, move, defender, field, isBadDreams) {
  if (isNaN(damage[0])) {
    return "something broke; please tell Pikalytics";
  }
  ...
  if (
    (damage.length !== 256 || (!hasSitrus && !hasFigy)) &&
    damage[0] >= defender.curHP
  ) {
    return "guaranteed OHKO";
  } else if (
    damage.length === 256 &&
    hasSitrus &&
    damage[0] >= defender.curHP + Math.floor(defender.maxHP / 4)
  ) {
    return "guaranteed OHKO";
  } else if (
    damage.length === 256 &&
    hasFigy &&
    damage[0] >= defender.curHP + Math.floor(defender.maxHP / 3)
  ) {
    return "guaranteed OHKO";
  }
  ...
  if (field.weather === "Sun") {
    ...
    if (
      ["Rock", "Ground", "Steel"].indexOf(defender.type1) === -1 &&
    ...
    if (defender.ability === "Ice Body") {
      ...
      defender.type1 !== "Ice" &&
      defender.type2 !== "Ice" &&
      ["Magic Guard", "Overcoat", "Snow Cloak"].indexOf(defender.ability) ===
        -1 &&
      defender.item !== "Safety Goggles"
    ) {
      eot -= Math.floor(defender.maxHP / 16);
      eotText.push("snow damage");
    }
  }
  ...
  if (field.terrain === "Grassy") {
    if (
      field.isGravity ||
      (defender.type1 !== "Flying" &&
        defender.type2 !== "Flying" &&
        defender.item !== "Air Balloon" &&
  ...
  var afterText =
    hazardText.length > 0 ? " after " + serializeText(hazardText) : "";
  if (c === 1) {
    return "guaranteed OHKO" + afterText;
  } else if (c > 0) {
    return (
      qualifier + Math.round(c * 1000) / 10 + "% chance to OHKO" + afterText
    );
  }
...
function getKOChance(
  ...
  if (hits === 1) {
    if ((!multihit || !hasSitrus) && maxDamage < hp) {
      ...
      multihit &&
      hasSitrus &&
      maxDamage < hp + Math.floor(maxHP / 4)
    ) {
      return 0;
    ...
    for (i = 0; i < n; i++) {
      if ((!multihit || (!hasSitrus && !hasFigy)) && damage[i] >= hp) {
        ...
        multihit &&
        hasSitrus &&
        damage[i] >= hp + Math.floor(maxHP / 4)
      ) {
        return (n - i) / n;
      } else if (
        multihit &&
        hasFigy &&
        damage[i] >= hp + Math.floor(maxHP / 3)
      ) {
        return (n - i) / n;
      }
    }
  ...
  for (i = 0; i < n; i++) {
    if (hp - damage[i] <= maxHP / 2 && hasSitrus) {
      ...
      (hp - damage[i] <= maxHP / 4 && hasFigy && !gluttony) ||
      (hp - damage[i] <= maxHP / 2 && hasFigy && gluttony)
    ) {
...
function predictTotal(
  ...
  for (var i = 0; i < hits; i++) {
    ...
    if (hp - total <= maxHP / 2 && hasSitrus) {
      ...
      (hp - total <= maxHP / 4 && hasFigy && !gluttony) ||
      (hp - total <= maxHP / 2 && hasFigy && gluttony)
    ) {
      hp += Math.floor(maxHP / 3);
      hasFigy = false;
    }
