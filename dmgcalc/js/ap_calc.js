...
function autosetWeather(ability, i) {
    ...
    if (
        lastAutoWeather.indexOf(currentWeather) === -1 ||
        currentWeather === ''
    ) {
    ...
    if (
        newWeather === 'Strong Winds' ||
        primalWeather.indexOf(newWeather) > -1
    ) {
...
$('.move-selector').change(function () {
    ...
    moveGroupObj
        .children('.move-crit')
        .prop('checked', move.alwaysCrit === true)
    if (move.isMultiHit) {
        moveGroupObj.children('.move-hits').show()
        moveGroupObj
            .children('.move-hits')
            .val(
                $(this).closest('.poke-info').find('.ability').val() ===
                    'Skill Link'
                    ? 5
                    : 1
            )
    ...
    if (pokemon) {
        ...
        if (pokemonName in setdex && setName in setdex[pokemonName]) {
            ...
            pokeObj
                .find('.hp .evs')
                .val(
                    set.evs && typeof set.evs.hp !== 'undefined'
                        ? set.evs.hp
                        : 0
                )
            pokeObj
                .find('.hp .ivs')
                .val(
                    set.ivs && typeof set.ivs.hp !== 'undefined'
                        ? set.ivs.hp
                        : 31
                )
            pokeObj
                .find('.hp .dvs')
                .val(
                    set.dvs && typeof set.dvs.hp !== 'undefined'
                        ? set.dvs.hp
                        : 15
                )
            for (i = 0; i < STATS.length; i++) {
            ...
            for (i = 0; i < 4; i++) {
                moveObj = pokeObj.find(
                    '.move' + (i + 1) + ' select.move-selector'
                )
...
function Pokemon(pokeInfo) {
    ...
    for (var i = 0; i < STATS.length; i++) {
        this.rawStats[STATS[i]] = ~~pokeInfo
            .find('.' + STATS[i] + ' .total')
            .text()
        this.boosts[STATS[i]] = ~~pokeInfo
            .find('.' + STATS[i] + ' .boost')
            .val()
        this.baseStats[STATS[i]] = ~~pokeInfo
            .find('.' + STATS[i] + ' .base')
            .val()
    ...
    $('.set-selector').select2({
        ...
        query: function (query) {
            ...
            for (var i = 0; i < setOptions.length; i++) {
                ...
                if (
                    !query.term ||
                    pokeName.indexOf(query.term.toUpperCase()) > -1
                ) {
