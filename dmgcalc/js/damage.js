...
function GET_DAMAGE_SM(attacker, defender, move, field, side1, side2) {
  ...
  if (
    ...
    attacker.name = attacker.name.replace("Mega ", "").replace(" X", "-Mega-X");
  } else if (
    attacker.name.indexOf("Mega Charizard Y") > -1 ||
    attacker.name.indexOf("Mega Mewtwo Y") > -1
  ) {
    attacker.name = attacker.name.replace("Mega ", "").replace(" Y", "-Mega-Y");
  ...
  if (
    ...
    defender.name = defender.name.replace("Mega ", "").replace(" X", "-Mega-X");
  } else if (
    defender.name.indexOf("Mega Charizard Y") > -1 ||
    defender.name.indexOf("Mega Mewtwo Y") > -1
  ) {
    defender.name = defender.name.replace("Mega ", "").replace(" Y", "-Mega-Y");
  ...
  var p1 = new calc.Pokemon(gen9, attacker.name, {
    ...
    ...(attacker.teraType ? { teraType: attacker.teraType } : {}),
    overrides: {
      baseStats: {
  ...
  var p2 = new calc.Pokemon(gen9, defender.name, {
    ...
    ...(defender.teraType ? { teraType: defender.teraType } : {}),
    overrides: {
      baseStats: {
...
function getFinalSpeedSM(pokemon, weather, terrain, side) {
  ...
  if (
    ...
    (pokemon.ability === "Slush Rush" && weather.indexOf("Snow") > -1) ||
...
function checkIntimidate(source, target) {
  if (source.ability === "Intimidate") {
    if (target.ability === "Contrary" || target.ability === "Defiant") {
      ...
      ["Clear Body", "White Smoke", "Hyper Cutter", "Full Metal Body"].indexOf(
        target.ability
      ) !== -1
    ) {
      // no effect
